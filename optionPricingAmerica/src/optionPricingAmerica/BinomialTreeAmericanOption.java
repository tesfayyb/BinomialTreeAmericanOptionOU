...
//	Ornstein-Uhlenbeck (OU) process.
	...
	public static double computeAmericanOption (double S_0, double mu, double sigma, double r,double beta,double K, double T, int n) 
	{
		...
		double [] americanPayoffs = new double [n + 1];  // array to store American put option payoffs
		...
		for(i = 0; i <= n; i++) {
			...
			americanPayoffs[i] = Math.max(K - s, 0.0);
		}
		...
		for(j = n - 1; j >= 0; j--) {
			for(i = 0; i <= j; i++) {
				...
				americanPayoffs[i] = Math.max(K - s, Math.exp(-r * dt) * (q * americanPayoffs[i] + (1 - q) * americanPayoffs[i + 1]));
			}
		}
		
		
		return americanPayoffs[0];
	}

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		double americanPutPrice = computeAmericanOption(45, 50, 0.3, 0.05, 0.5, 50, 1, 1000);
		
		System.out.println( "The price of American put price is :   " + americanPutPrice);
